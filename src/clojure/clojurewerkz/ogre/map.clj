(ns clojurewerkz.ogre.map
  ...
  (:import (org.apache.tinkerpop.gremlin.process.traversal.dsl.graph GraphTraversal)
           (org.apache.tinkerpop.gremlin.structure Order Element)
           (org.apache.tinkerpop.gremlin.process.traversal.step.map MapStep)
           (org.apache.tinkerpop.gremlin.process.traversal Traverser))
  (:require [clojurewerkz.ogre.util :refer (f-to-function fs-to-function-array keywords-to-str-array keywords-to-str-list f-to-bifunction typed-traversal as)]))

(defn back
  "Goes back to the results of a named step."
  ([^GraphTraversal t step-label] (typed-traversal .back t (name step-label))))
...
(defn fold
  ...
  ([^GraphTraversal t] (typed-traversal .fold t)))
...
(defn local
  ...
  [^GraphTraversal t local-t] (typed-traversal .local t local-t))
...
(defmacro match
  ...
  [^GraphTraversal t start-label & matches]
...
(defn order
  ...
  [^GraphTraversal t] (.order t))

(defn by
  ([^GraphTraversal t]
    (typed-traversal .by t))
  ([^GraphTraversal t arg]
...
(defn other-v
  ...
  ([^GraphTraversal t] (typed-traversal .otherV t)))
...
(defn shuffle
  ...
  ([^GraphTraversal t] (typed-traversal .shuffle t)))

;; to

(defn unfold
  "Unroll all objects in the iterable at the current step."
  ([^GraphTraversal t] (typed-traversal .unfold t)))
